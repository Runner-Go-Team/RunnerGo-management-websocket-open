...
func RegisterRouter(r *gin.Engine) {
	...
	auth.POST("/login", handler.AuthLogin)              //手机号和邮箱密码登录二合一
	...
	team.GET("list_new", handler.ListTeam)
	...
	target.POST("clone_api", handler.CloneApi) // 复制接口
	...
	plan.POST("get_estimate_use_vum_num", handler.GetEstimateUseVumNum) // 获取当前计划预估使用vum数量
