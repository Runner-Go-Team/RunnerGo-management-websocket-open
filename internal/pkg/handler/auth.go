...
func RegisterOrLogin(ctx *gin.Context) {
	...
	if err != nil { // 没注册过
		userInfo, err = auth.ToSignUp(ctx, req.Mobile, req.Ver, inviteInfoArr)
		if err != nil {
	...
	// 把token设置为过期
	_, _, err = jwt.GenerateTokenByTime(userInfo.UserID, 0)
...
func AuthSignup(ctx *gin.Context) {
	...
	mobileCnt, mobileErr := tx.WithContext(ctx).Where(tx.Mobile.Eq(req.Mobile)).Count()
	if mobileErr != nil {
		response.ErrorWithMsg(ctx, errno.ErrMysqlFailed, mobileErr.Error())
		return
	}
	if mobileCnt > 0 {
	...
	u, err := auth.SignUp(ctx, req.Email, req.Mobile, req.Password, req.Nickname)
	if err != nil {
		response.ErrorWithMsg(ctx, errno.ErrMysqlFailed, err.Error())
		return
	}

	token, exp, err := jwt.GenerateToken(u.UserID)
	if err != nil {
		response.ErrorWithMsg(ctx, errno.ErrInvalidToken, err.Error())
		return
	}
...
func AuthLogin(ctx *gin.Context) {
	...
	if req.Email == "" && req.Mobile == "" {
		response.ErrorWithMsg(ctx, errno.ErrParam, "邮箱或手机号不能同时为空")
		return
	}
	...
	if err != nil {
		...
		//if err.Error() == "验证失败" {
		//	response.ErrorWithMsg(ctx, errno.ErrVerifyFail, err.Error())
		//}
		return
	}
...
func SetUserSettings(ctx *gin.Context) {
	...
	if err := auth.SetUserSettings(ctx, jwt.GetUserIDByCtx(ctx), &req.UserSettings); err != nil {
		response.ErrorWithMsg(ctx, errno.ErrTeamOverdue, "")
		return
	}
...
func GetUserSettings(ctx *gin.Context) {
	...
	if res.UserSettings.CurrentTeamID == "" {
		response.ErrorWithMsg(ctx, errno.ErrNotFundAvailTeam, "")
		return
	}
