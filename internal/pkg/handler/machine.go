...
import (
	...
	"RunnerGo-management/internal/pkg/logic/machine"
	"RunnerGo-management/internal/pkg/logic/stress"
...
func MachineDataInsert() {
	...
	for {
		...
		if len(machineListRes.Val()) == 0 || machineListRes.Err() != nil {
			proof.Infof("压力机数据入库--没有获取到任何压力机上报数据，err:", machineListRes.Err())
		...
		for machineAddr, machineDetail := range machineListRes.Val() {
			...
			if err != nil {
				proof.Infof("压力机数据入库--转换类型失败，err:", err)
				continue
			}
			...
			if err != nil && err != gorm.ErrRecordNotFound {
				proof.Infof("压力机数据入库--查询数据出错，err:", err)
				continue
			}
			...
			if err == nil { // 查到了，修改数据
				...
				if err != nil {
					proof.Infof("压力机数据入库--更新数据失败，err:", err)
					continue
				}
			} else { // 没查到，新增数据
...
func MachineMonitorInsert() {
	...
	for {
		...
		for _, MachineMonitorKey := range machineList {
			...
			if err.Err() != nil {
				proof.Infof("压力机监控数据入库--删除redis列表失败，err:", err.Err())
			}
		}
...
func InitTotalKafkaPartition() {
	...
	//组装总共需要初始化的分区数组
	totalKafkaPartitionArr := make([]interface{}, 0, 500)
	for i := 0; i < 500; i++ {
