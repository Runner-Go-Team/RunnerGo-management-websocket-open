...
func GetInviteUserInfo(ctx *gin.Context) {
	...
	if err != nil {
		if err.Error() == "团队人数已超上限" || err.Error() == "团队不存在或已解散" {
			response.ErrorWithMsg(ctx, errno.ErrTeamUserOvertopLimit, err.Error())
...
func GetInviteEmailIsExist(ctx *gin.Context) {
	...
	if err != nil {
		if err.Error() == "受邀人数已超出团队成员限额" {
			response.ErrorWithMsg(ctx, errno.ErrInviteUserNumOvertopLimit, err.Error())
		} else {
			response.ErrorWithMsg(ctx, errno.ErrMysqlFailed, err.Error())
		}
		return
