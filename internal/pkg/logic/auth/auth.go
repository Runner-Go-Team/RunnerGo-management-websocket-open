...
func SignUp(ctx context.Context, email, mobile, password, nickname string) (*model.User, error) {
...
func Login(ctx *gin.Context, req rao.AuthLoginReq) (*model.User, error) {
	//// 先验证极验验证码
	//geeTestRes := FastVerify(req.Captcha)
	//if geeTestRes == false {
	...
	var err error
	if req.Mobile != "" {
		user, err = tx.WithContext(ctx).Where(tx.Mobile.Eq(req.Mobile)).First()
		if err != nil {
...
func AddUserEverydayActivity(ctx *gin.Context) error {
	userID := jwt.GetUserIDByCtx(ctx)
	if userID == "" {
		return nil
	}

	key := "UserEverydayActivity:" + userID
	userActRes := dal.GetRDB().SetNX(ctx, key, 1, time.Second*2)
	if userActRes.Val() == false {
		return userActRes.Err()
	}

	timeTemp, _ := time.ParseInLocation("2006-01-02", time.Now().Format("2006-01-02"), time.Local)
	startTime := timeTemp.Unix()
	endTime := timeTemp.Unix() + (3600*24 - 1)

	collection := dal.GetMongo().Database(dal.MongoDB()).Collection(consts.CollectUserEverydayActivity)
	var detail mao.UserEverydayActivity
	err := collection.FindOne(ctx, bson.D{{"user_id", userID},
		{"created_at_int", bson.D{{"$gte", startTime}}}, {"created_at_int", bson.D{{"$lte", endTime}}}}).Decode(&detail)
	if err == nil {
		return nil
	}
	...
	operationLog := mao.UserEverydayActivity{
		UserID:       userID,
		CreatedAtInt: nowTimeInt,
		CreatedAt:    nowTimeDate,
	}
