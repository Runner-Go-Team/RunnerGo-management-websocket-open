...
func CheckTeamIsOverdue(baton *Baton) (int, error) {
	teamIsOverdue, _ := team.GetTeamIsOverdue(baton.Ctx, baton.TeamID)
	if teamIsOverdue == true {
		return errno.ErrTeamOverdue, fmt.Errorf("当前团队已过期")
	}
	return errno.Ok, nil
}
