...
func SaveTask(ctx *gin.Context, req *rao.SavePlanConfReq, userID string) (int, error) {
	...
	if req.TaskType == consts.PlanTaskTypeNormal { // 普通任务
		//限制轮次数
		if req.ModeConf.RoundNum > 1000 {
			return errno.ErrRoundNumOvertopLimit, fmt.Errorf("保存失败")
		}
	} else {
		// 结束时间不可晚于团队过期时间
		if req.TimedTaskConf.Frequency == 0 {
			if teamInfo.VipExpirationDate.Unix() < req.TimedTaskConf.TaskExecTime {
				return errno.ErrTimedTaskTimeGtTeamTime, fmt.Errorf("定时任务时间不可晚于团队过期时间")
			}
		} else {
			if teamInfo.VipExpirationDate.Unix() < req.TimedTaskConf.TaskCloseTime {
				return errno.ErrTimedTaskTimeGtTeamTime, fmt.Errorf("定时任务时间不可晚于团队过期时间")
			}
		}
	}
	...
	if req.TaskType == consts.PlanTaskTypeNormal { // 普通任务
		err = query.Use(dal.DB()).Transaction(func(tx *query.Query) error {
			...
			if err == nil { // 已存在 则修改
				updateData := make(map[string]interface{}, 5)
				updateData["task_type"] = req.TaskType
				updateData["task_mode"] = req.Mode
				updateData["control_mode"] = req.ControlMode
				updateData["mode_conf"] = string(modeConfString)
...
func GetEstimateUseVumNum(ctx *gin.Context, req *rao.GetEstimateUseVumNumReq) (rao.GetEstimateUseVumNumResp, error) {
	...
	allErr := dal.GetQuery().Transaction(func(tx *query.Query) error {
		...
		sceneList, err := tx.Target.WithContext(ctx).Where(tx.Target.TeamID.Eq(req.TeamID),
			tx.Target.PlanID.Eq(req.PlanID), tx.Target.Source.Eq(consts.TargetSourcePlan),
			tx.Target.TargetType.Eq(consts.TargetTypeScene)).Find()
		if err != nil {
			return err
		}
		...
		if planInfo.TaskType == consts.PlanTaskTypeNormal { // 普通计划
			// 查询普通计划下面所有任务配置信息
			stressPlanTaskConfList, err := tx.StressPlanTaskConf.WithContext(ctx).Where(tx.StressPlanTaskConf.TeamID.Eq(req.TeamID),
				tx.StressPlanTaskConf.PlanID.Eq(req.PlanID)).Find()
			if err != nil {
				return err
			}

			if len(stressPlanTaskConfList) == 0 || len(sceneList) != len(stressPlanTaskConfList) {
				return fmt.Errorf("没有查到任务配置")
			}

			for _, taskInfo := range stressPlanTaskConfList {
				var modeConf rao.ModeConf
				err := json.Unmarshal([]byte(taskInfo.ModeConf), &modeConf)
				if err != nil {
					return fmt.Errorf("任务配置错误")
				}
				// 持续时长分钟数
				var runMinute int64 = 0
				if modeConf.Duration != 0 {
					runMinute = int64(math.Ceil(float64(modeConf.Duration) / 60))
				} else {
					runMinute = int64(math.Ceil(float64(modeConf.RoundNum) / 60))
				}

				// 判断施压模式
				var vumTemp int64 = 0
				if taskInfo.TaskMode == consts.PlanModeConcurrence {
					vumTemp = runMinute * modeConf.Concurrency
				} else {
					// 把时长转换为分钟，向上取整
					vumTemp = GetVumTotalNum(modeConf)
				}
				maybeUseVumNum = maybeUseVumNum + vumTemp
			}
		} else { // 定时计划
			// 查询普通计划下面所有任务配置信息
			stressPlanTimedTaskConfList, err := tx.StressPlanTimedTaskConf.WithContext(ctx).Where(tx.StressPlanTimedTaskConf.TeamID.Eq(req.TeamID),
				tx.StressPlanTimedTaskConf.PlanID.Eq(req.PlanID)).Find()
			if err != nil {
				return err
			}

			if len(stressPlanTimedTaskConfList) == 0 || len(sceneList) != len(stressPlanTimedTaskConfList) {
				return fmt.Errorf("没有查到任务配置")
			}

			for _, taskInfo := range stressPlanTimedTaskConfList {
				var modeConf rao.ModeConf
				err := json.Unmarshal([]byte(taskInfo.ModeConf), &modeConf)
				if err != nil {
					return fmt.Errorf("任务配置错误")
	...
	res := rao.GetEstimateUseVumNumResp{
		MaybeUseVumNum:   maybeUseVumNum,
		TeamUsableVumNum: teamUsableVumNum,
	}
	return res, allErr
}
